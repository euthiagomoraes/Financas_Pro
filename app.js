...
const state={contas:[],recorrentes:[],emprestimos:[],parcelas:[],categorias:[],assinaturas:[],assinaturaCobrancas:[],metas:[],metaItens:[]};
const SERVICE_LOGOS={
...
function nav(){return [['dashboard','layout-dashboard','Início'],['contas','receipt-text','Contas'],['emprestimos','hand-coins','Empréstimos'],['calendario','calendar-days','Calendário'],['assinaturas','repeat-2','Assinaturas'],['metas','target','Metas'],['relatorios','chart-no-axes-combined','Relatórios'],['familia','users','Área da Família'],['perfil','circle-user-round','Perfil']];}
...
async function deleteRow(table,id,msg){if(!confirm('Excluir este registro?'))return;const {data,error}=await sb.from(table).delete().eq('id',id).eq('usuario_id',user.id).select('id');if(error)return toast(error.message);if(!data?.length)return toast('Nenhum registro foi excluído.');await loadData();render();toast(msg)}
...
async function loadData(){
 ...
 const [c,l,p,sub,charges,goals,goalItems]=await Promise.all([
