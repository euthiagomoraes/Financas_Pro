...
const defaultState = {
  ...
  contas: [
    ...
    {id:3, descricao:"\u00C1gua", categoria:"Moradia", valor:180, vencimento:"2026-09-25", status:"Pendente", pagoEm:"", valorPago:0},
    ...
    {id:6, descricao:"Supermercado", categoria:"Alimenta\u00E7\u00E3o", valor:320, vencimento:"2026-09-04", status:"Pendente", pagoEm:"", valorPago:0},
    {id:7, descricao:"Condom\u00EDnio", categoria:"Moradia", valor:980, vencimento:"2026-09-06", status:"Pago", pagoEm:"2026-09-06", valorPago:980},
    {id:8, descricao:"Plano de sa\u00FAde", categoria:"Sa\u00FAde", valor:450, vencimento:"2026-09-08", status:"Pago", pagoEm:"2026-09-08", valorPago:450}
  ],
  emprestimos: [
    {id:1, descricao:"Empr\u00E9stimo Banco X", credor:"Banco X", total:20000, parcelas:48, valorParcela:650, primeira:"2026-10-10", status:"Ativo",
...
function setRoute(r){
  ...
  const meta={
    dashboard:["Ol\u00E1, Jessyca! \uD83D\uDC4B","Veja como est\u00E3o suas finan\u00E7as hoje."],
...
function dashboardHTML(){
  ...
  return `<div class="grid kpi-grid">
    <div class="kpi"><div class="kpi-head"><div class="kpi-icon soft-green">\u2302</div></div><label>Total de contas do m\u00EAs</label><strong>${money(total)}</strong><small>${state.contas.length} contas</small><div class="progress"><span style="width:${Math.min(100,total?paid/total*100:0)}%"></span></div></div>
    <div class="kpi"><div class="kpi-head"><div class="kpi-icon soft-green">\u2713</div></div><label>Pagas</label><strong>${money(paid)}</strong><small>${state.contas.filter(isPaid).length} contas (${Math.round(total?paid/total*100:0)}%)</small><div class="progress"><span style="width:${Math.min(100,total?paid/total*100:0)}%"></span></div></div>
    <div class="kpi"><div class="kpi-head"><div class="kpi-icon soft-red">\u25F7</div></div><label>Pendentes</label><strong>${money(pending)}</strong><small>${state.contas.filter(c=>!isPaid(c)).length} contas (${Math.round(total?pending/total*100:0)}%)</small><div class="progress"><span style="width:${Math.min(100,total?pending/total*100:0)}%;background:#ff6473"></span></div></div>
    <div class="kpi"><div class="kpi-head"><div class="kpi-icon soft-purple">\u25A4</div></div><label>Empr\u00E9stimos</label><strong>${money(loans)}</strong><small>Total a pagar (saldo)</small><div class="progress"><span style="width:${state.emprestimos.length?55:0}%;background:#7564e9"></span></div></div>
  </div>
  <div class="grid content-grid">
    <div class="panel"><div class="panel-head"><div><h3>Despesas por categoria</h3><div class="panel-sub">Este m\u00EAs</div></div></div><div class="donut-area"><div class="donut"></div><div class="legend">${catRows}</div></div></div>
    <div class="panel"><div class="panel-head"><div><h3>Evolu\u00E7\u00E3o dos gastos</h3><div class="panel-sub">\u00DAltimos 6 meses</div></div></div><div class="chart-wrap">${lineChart()}</div></div>
  ...
  <div class="grid bottom-grid">
    <div class="panel"><div class="panel-head"><h3>Pr\u00F3ximos vencimentos</h3><button class="link-btn" data-route="contas">Ver todos \u2192</button></div><div class="list">${upcoming.length?upcoming.map(c=>rowConta(c)).join(""):`<div class="empty">Nenhuma conta pendente.</div>`}</div></div>
    <div class="panel"><div class="panel-head"><h3>Empr\u00E9stimos</h3><button class="link-btn" data-route="emprestimos">Ver todos \u2192</button></div>${state.emprestimos.length?loanSummary(state.emprestimos[0]):`<div class="empty"><strong>Nenhum empr\u00E9stimo</strong>Cadastre seu primeiro empr\u00E9stimo.</div>`}</div>
    <div class="panel"><div class="panel-head"><h3>Resumo financeiro</h3></div><div style="background:#f1f7fa;padding:13px;border-radius:9px"><small style="font-size:10px;color:#63809c">Saldo previsto do m\u00EAs</small><strong style="display:block;font-size:21px;margin-top:6px">${money(total-paid)}</strong></div><div style="background:#e6f6f4;padding:13px;border-radius:9px;margin-top:12px;font-size:11px;color:#45617b">\uD83D\uDC37 <b>Voc\u00EA est\u00E1 no controle!</b><br><span style="font-size:10px">Continue assim e mantenha suas finan\u00E7as saud\u00E1veis.</span></div></div>
  </div>
  <div class="panel" style="margin-top:14px"><div class="panel-head"><h3>\u00DAltimas movimenta\u00E7\u00F5es</h3><button class="link-btn" data-route="contas">Ver todas \u2192</button></div><div class="list">${[...state.contas].sort((a,b)=>(b.pagoEm||b.vencimento).localeCompare(a.pagoEm||a.vencimento)).slice(0,3).map(c=>rowConta(c,true)).join("")}</div></div>`;
}
function lineChart(){
...
function rowConta(c, movement=false){return `<div class="list-row"><div class="item-icon">${c.categoria==="Moradia"?"\u26A1":c.categoria==="Internet"?"\u2301":"\u25C9"}</div><div><strong>${esc(c.descricao)}</strong><small>${esc(c.categoria)} \u00B7 ${movement&&isPaid(c)?dateBR(c.pagoEm):dateBR(c.vencimento)}</small></div><div class="amount">${money(c.valor)}</div><span class="badge ${isPaid(c)?"paid":"pending"}">${isPaid(c)?"Pago":"Pendente"}</span></div>`}
function loanSummary(e){const pc=paidCount(e),rem=remainingLoan(e),pct=Math.round(pc/e.parcelas*100);return `<div class="loan-card"><div class="loan-top"><div class="loan-name">\u25A4 ${esc(e.descricao)}</div><span class="amount">${money(totalParcelas(e))}<small style="display:block;color:#63809c;font-weight:400">Total a pagar</small></span></div><div class="loan-values"><div><strong>${money(e.total)}</strong><small>Valor contratado</small></div></div><div class="loan-bar"><span style="width:${pct}%"></span></div><div style="display:flex;justify-content:space-between;font-size:9px;color:#66809d"><span>${pc} de ${e.parcelas} parcelas pagas</span><b>${pct}%</b></div><div class="loan-meta"><div><small>Pago</small><strong class="green">${money(paidLoan(e))}</strong></div><div><small>Restante</small><strong class="red">${money(rem)}</strong></div><div><small>Pr\u00F3xima parcela</small><strong>${e.primeira?dateBR(e.primeira):"\u2014"}</strong><small>${money(e.valorParcela)}</small></div></div><button class="btn btn-outline btn-block" style="margin-top:12px" data-loan-view="${e.id}">Ver detalhes</button></div>`}
function miniCalendarHTML(){
  ...
  return `<div class="panel-head"><div><h3>Calend\u00E1rio de pagamentos</h3><div class="panel-sub">${new Intl.DateTimeFormat("pt-BR",{month:"long",year:"numeric"}).format(calendarDate)}</div></div><span>\u2039\u3000\u203A</span></div><table><thead><tr>${["D","S","T","Q","Q","S","S"].map(x=>`<th>${x}</th>`).join("")}</tr></thead><tbody><tr>${cells}</tr></tbody></table><div class="cal-legend"><span><i class="dot" style="background:#10b88d"></i>Pago</span><span><i class="dot" style="background:#ff6473"></i>Pendente</span><span><i class="dot" style="background:#f4c64e"></i>Vencendo hoje</span><span><i class="dot" style="background:#7c91a3"></i>Vencido</span></div>`;
}
...
function contasHTML(){
  ...
  return `<div class="toolbar"><div><h2>Contas da casa</h2><p>Contas s\u00E3o mantidas separadas dos empr\u00E9stimos.</p></div><div class="toolbar-actions"><input id="contaSearch" class="search" placeholder="Buscar conta..."><button class="btn btn-primary" data-new-conta>+ Nova conta</button></div></div><div class="panel table-panel"><table class="data-table"><thead><tr><th>Conta</th><th>Valor</th><th>Vencimento</th><th>Status</th><th style="text-align:right">A\u00E7\u00F5es</th></tr></thead><tbody>${rows||`<tr><td colspan="5"><div class="empty"><strong>Nenhuma conta cadastrada</strong>Adicione sua primeira conta.</div></td></tr>`}</tbody></table></div>`;
}
...
function emprestimosHTML(){
  ...
  return `<div class="toolbar"><div><h2>Empr\u00E9stimos</h2><p>Controle separado das contas da casa.</p></div><button class="btn btn-primary" data-new-loan>+ Novo empr\u00E9stimo</button></div><div class="page-grid">${cards||`<div class="panel empty"><strong>Nenhum empr\u00E9stimo cadastrado</strong>Cadastre um empr\u00E9stimo para gerar automaticamente suas parcelas.</div>`}</div>`;
}
...
function newConta(id=null){
  ...
  openModal(id?"Editar conta":"Nova conta",`<div class="form-grid">
    <div class="field full"><label>Descri\u00E7\u00E3o<input name="descricao" required value="${esc(c.descricao||"")}"></label></div>
    <div class="field"><label>Categoria<select name="categoria">${["Moradia","Alimenta\u00E7\u00E3o","Transporte","Sa\u00FAde","Educa\u00E7\u00E3o","Internet","Lazer","Outros"].map(x=>`<option ${x===c.categoria?"selected":""}>${x}</option>`).join("")}</select></label></div>
    ...
    <div class="field"><label>Observa\u00E7\u00E3o<input name="obs" value="${esc(c.obs||"")}"></label></div>
...
function newLoan(){
  openModal("Novo empr\u00E9stimo",`<div class="form-grid">
    <div class="field full"><label>Descri\u00E7\u00E3o<input name="descricao" placeholder="Ex.: Empr\u00E9stimo Banco X" required></label></div>
    <div class="field"><label>Credor / institui\u00E7\u00E3o<input name="credor" placeholder="Banco X" required></label></div>
    ...
    <div class="field full"><label>Observa\u00E7\u00E3o<textarea name="obs"></textarea></label></div>
  </div>`,`Cadastrar empr\u00E9stimo`,f=>{
    ...
    state.emprestimos.push(e);save();render();toast("Empr\u00E9stimo cadastrado e parcelas geradas");
  });
}
