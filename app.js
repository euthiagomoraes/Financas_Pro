...
let state = { loggedIn:false, profile:{name:"Usuário",email:""}, contas:[], emprestimos:[] };
...
function normalizeLoan(l, parcelas=[]){
  ...
  return {id:l.id,descricao:l.descricao||l.nome||"Empréstimo",credor:l.credor||l.instituicao||"",total:Number(l.valor_total||l.total||l.valor_contratado||0),parcelas:Number(l.numero_parcelas||l.parcelas||0),valorParcela:Number(l.valor_parcela||l.parcela||0),primeira:isoDate(l.primeira_data_vencimento||l.primeira||l.data_primeiro_vencimento),status:l.status||"Ativo",pagamentos:ps};
}
...
async function loadFinanceData(){
  ...
  const [contasRes, loansRes, parcelasRes] = await Promise.all([
...
async function startSession(session){
  ...
  state.loggedIn=true; state.profile=await loadProfile(currentUser); state.contas=[]; state.emprestimos=[];
  showApp(); render();
...
$("#logoutBtn")?.addEventListener("click",async()=>{await supabaseClient.auth.signOut();state={loggedIn:false,profile:{name:"Usuário",email:""},contas:[],emprestimos:[]};showLogin();});
