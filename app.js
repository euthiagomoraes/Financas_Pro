...
let user=null, profile=null, page='dashboard', contaFilter='all', loanFilter='Ativos', calendarDate=new Date(), sidebarCollapsed=localStorage.getItem('financas-sidebar-collapsed')==='1', contasMenuOpen=localStorage.getItem('financas-contas-menu-open')==='1', contaFilterPanelOpen=false;
const contaListFilter={status:'all',origin:'all',category:'all',sort:'date-desc'};
...
function nav(){return [['emprestimos','hand-coins','Empréstimos'],['calendario','calendar-days','Calendário'],['assinaturas','repeat-2','Assinaturas'],['metas','target','Metas'],['relatorios','chart-no-axes-combined','Relatórios'],['familia','users','Área da Família'],['perfil','circle-user-round','Perfil']];}
function contasNav(){const inContas=['contas','contas-parceladas'].includes(page);const open=contasMenuOpen;return `<div class="nav-group ${open?'open':''}"><button class="nav-group-toggle ${inContas?'active':''}" data-action="toggle-contas-menu" aria-expanded="${open}">${icon('receipt-text',18)}<span>Contas</span><span class="nav-chevron">${icon(open?'chevron-down':'chevron-right',14)}</span></button>${open?`<div class="nav-sub"><button data-nav="contas" class="${page==='contas'?'active':''}">${icon('list',15)}<span>Todas</span></button><button data-nav="contas-parceladas" class="${page==='contas-parceladas'?'active':''}">${icon('layers-3',15)}<span>Parceladas</span></button></div>`:''}</div>`;}
function familySelector(){return ''}
function shell(){return `<div class="mobile-overlay" id="mobileOverlay" aria-hidden="true"></div><aside class="sidebar" id="sidebar"><button class="sidebar-collapse" id="sidebarCollapse" title="Recolher/expandir menu" aria-label="Recolher ou expandir menu">${icon("panel-left-close",18)}</button><div class="brand"><div class="brand-mark">${icon('trending-up',23)}</div><div><h2>Finanças Pro</h2><span>Você no controle</span></div></div><nav class="nav"><button data-nav="dashboard" class="${page==='dashboard'?'active':''}">${icon('layout-dashboard',18)}<span>Início</span></button>${contasNav()}${nav().map(([p,ic,l])=>`<button data-nav="${p}" class="${page===p?'active':''}">${icon(ic,18)}<span>${l}</span></button>`).join('')}</nav><div class="sidebar-spacer"></div><div class="quote">“Grandes objetivos começam com um bom planejamento financeiro.”</div><div class="user-mini"><div class="avatar">${avatarUrl()?`<img src="${esc(avatarUrl())}" alt="Foto do perfil">`:icon('user',20)}</div><div><strong>${esc(name())}</strong><span>${esc(user?.email||profile?.email||'')}</span></div><button class="logout" id="logout" title="Sair">${icon('log-out',17)}</button></div></aside><main class="main"><header class="topbar"><button class="mobile-menu" id="mobileMenu">${icon('menu',21)}</button><div class="top-actions"><div class="month-selector" aria-label="Competência selecionada"><button class="icon-btn month-selector-btn" data-month="-1" aria-label="Mês anterior">${icon('chevron-left',16)}</button><div class="month-selector-label" aria-label="Mês selecionado">${icon('calendar-days',15)}<span>${monthName(calendarDate)}</span></div><button class="icon-btn month-selector-btn" data-month="1" aria-label="Próximo mês">${icon('chevron-right',16)}</button></div>${notificationBell()}<button class="icon-btn" data-nav="perfil"><div class="avatar" style="width:30px;height:30px">${avatarUrl()?`<img src="${esc(avatarUrl())}" alt="">`:icon('user',16)}</div></button></div></header><div class="content" id="content"></div></main><nav class="mobile-bottom">${[['dashboard','house','Início'],['contas','receipt-text','Contas'],['calendario','calendar-days','Calendário'],['emprestimos','hand-coins','Empréstimos'],['assinaturas','repeat-2','Mais']].map(([p,ic,l])=>`<button data-nav="${p}" class="${page===p?'active':''}">${icon(ic,19)}<span>${l}</span></button>`).join('')}</nav>`}
...
function dashboard(){
 ...
 const dueDate=value=>{const x=new Date(String(value)+'T12:00:00');x.setHours(0,0,0,0);return x};
 const bills=[
  ...state.contas.filter(c=>inMonth(c.vencimento)),
  ...state.parcelas.filter(p=>inMonth(p.data_vencimento)).map(p=>({id:`loan-${p.id}`,descricao:`Parcela ${p.numero_parcela}`,valor:Number(p.valor)||0,vencimento:p.data_vencimento,status:String(p.status||'pendente').toLowerCase(),origem:'Empréstimo',virtual:true})),
  ...state.assinaturaCobrancas.filter(ch=>inMonth(ch.data_vencimento)).map(ch=>({id:`subcharge-${ch.id}`,descricao:state.assinaturas.find(s=>s.id===ch.assinatura_id)?.servico||'Assinatura',valor:Number(ch.valor)||0,vencimento:ch.data_vencimento,status:String(ch.status||'pendente').toLowerCase(),origem:'Assinatura',virtual:true})),
  ...state.parceladaItens.filter(p=>inMonth(p.data_vencimento)).map(p=>({id:`parcelada-${p.id}`,descricao:state.parceladas.find(x=>x.id===p.conta_parcelada_id)?.descricao||'Conta parcelada',valor:Number(p.valor)||0,vencimento:p.data_vencimento,status:String(p.status||'pendente').toLowerCase(),origem:'Parcelada',virtual:true}))
 ];
 const total=bills.reduce((s,c)=>s+Number(c.valor||0),0);
 const paid=bills.filter(isPaid);
 const unpaid=bills.filter(c=>!isPaid(c));
 const paidTotal=paid.reduce((s,c)=>s+Number(c.valor||0),0);
 const notDue=unpaid.filter(c=>dueDate(c.vencimento)>=todayDate);
 const overdue=unpaid.filter(c=>dueDate(c.vencimento)<todayDate);
 const notDueTotal=notDue.reduce((s,c)=>s+Number(c.valor||0),0);
 const overdueTotal=overdue.reduce((s,c)=>s+Number(c.valor||0),0);
 ...
 const upcoming=notDue.sort((a,b)=>String(a.vencimento).localeCompare(String(b.vencimento))).slice(0,5).map(c=>({date:c.vencimento,title:c.descricao,value:c.valor,type:c.origem}));
 return `<section class="welcome"><div><div class="eyebrow">Visão geral</div><h1>Olá, ${esc(name())}!</h1><p>Resumo de ${monthName(d).toLowerCase()}.</p></div><button class="btn btn-primary" data-action="new-conta">${icon('plus',17)} Lançar conta</button></section><section class="kpis"><article class="card kpi green dashboard-link" data-dashboard-route="all" tabindex="0" role="button" aria-label="Abrir todas as contas de ${monthName(d)}"><div class="kpi-icon">${icon('wallet',19)}</div><small>Contas do mês</small><h3>${money(total)}</h3><p>${bills.length} registro(s)</p></article><article class="card kpi green dashboard-link" data-dashboard-route="paid" tabindex="0" role="button" aria-label="Abrir contas pagas"><div class="kpi-icon">${icon('circle-check',19)}</div><small>Contas pagas</small><h3>${money(paidTotal)}</h3><p>${paid.length} paga(s)</p></article><article class="card kpi red dashboard-link" data-dashboard-route="pending" tabindex="0" role="button" aria-label="Abrir contas ainda não vencidas"><div class="kpi-icon">${icon('clock-3',19)}</div><small>Contas a pagar</small><h3>${money(notDueTotal)}</h3><p>${notDue.length} não vencida(s)</p></article><article class="card kpi gold dashboard-link" data-dashboard-route="overdue" tabindex="0" role="button" aria-label="Abrir contas vencidas"><div class="kpi-icon">${icon('triangle-alert',19)}</div><small>Vencidas</small><h3>${money(overdueTotal)}</h3><p>${overdue.length} vencida(s)</p></article></section><div class="grid-main"><section class="card section-card"><div class="section-head"><div><h2>Resumo financeiro</h2><p>Últimos 6 meses até ${monthName(d)}</p></div><div class="legend"><span><i class="dot green"></i>Contas pagas</span><span><i class="dot red"></i>Contas lançadas</span></div></div>${monthChart()}</section><section class="card section-card"><div class="section-head"><div><h2>Resumo por categoria</h2><p>${monthName(d)}</p></div></div>${dashboardCategorySummary(bills,monthLoans,monthSubs)}</section></div><div class="grid-main"><section class="card section-card"><div class="section-head"><div><h2>Próximos vencimentos</h2><p>Contas ainda não vencidas</p></div><button class="link-btn" data-nav="calendario">Ver calendário →</button></div><div class="upcoming">${upcoming.length?upcoming.map(u=>`<div class="up-item"><div class="date-box"><strong>${new Date(u.date+'T12:00:00').getDate()}</strong><small>${new Date(u.date+'T12:00:00').toLocaleDateString('pt-BR',{month:'short'}).replace('.','')}</small></div><div class="up-copy"><strong>${esc(u.title)}</strong><small>${money(u.value)} • ${esc(u.type)}</small></div><span class="status pending">Pendente</span></div>`).join(''):'<div class="empty">Nenhum vencimento futuro pendente.</div>'}</div></section><section class="card section-card"><div class="section-head"><div><h2>Vencidas</h2><p>Contas que já venceram no mês</p></div><button class="link-btn" data-dashboard-route="overdue">Ver todas →</button></div><div class="upcoming">${overdue.slice(0,5).map(c=>`<div class="up-item"><div class="date-box"><strong>${new Date(c.vencimento+'T12:00:00').getDate()}</strong><small>${new Date(c.vencimento+'T12:00:00').toLocaleDateString('pt-BR',{month:'short'}).replace('.','')}</small></div><div class="up-copy"><strong>${esc(c.descricao)}</strong><small>${money(c.valor)} • ${esc(c.origem)}</small></div><span class="status pending">Vencida</span></div>`).join('')||'<div class="empty">Nenhuma conta vencida no mês.</div>'}</div></section></div>`}
...
function dashboardCategorySummary(bills,loanTotal,subTotal){
 ...
 const commitmentRows=`<div class="metric-row"><span>Empréstimos</span><div class="metric-bar"><i style="width:${Math.min(100,loanTotal>0?60:0)}%"></i></div><strong>${money(loanTotal)}</strong></div><div class="metric-row"><span>Assinaturas</span><div class="metric-bar"><i style="width:${Math.min(100,subTotal>0?35:0)}%"></i></div><strong>${money(subTotal)}</strong></div>`;
 return `<div class="metric-list">${rows||'<div class="empty">Nenhuma categoria lançada neste mês.</div>'}${commitmentRows}</div>`;
}

function monthChart(){const end=new Date(calendarDate.getFullYear(),calendarDate.getMonth(),1),arr=[];for(let i=5;i>=0;i--){const d=new Date(end.getFullYear(),end.getMonth()-i,1),y=d.getFullYear(),m=d.getMonth();const matches=value=>{const x=new Date(String(value)+'T12:00:00');return x.getFullYear()===y&&x.getMonth()===m};const rows=[...state.contas.filter(c=>matches(c.vencimento)),...state.parcelas.filter(p=>matches(p.data_vencimento)).map(p=>({valor:Number(p.valor)||0,status:String(p.status||'pendente').toLowerCase()})),...state.assinaturaCobrancas.filter(ch=>matches(ch.data_vencimento)).map(ch=>({valor:Number(ch.valor)||0,status:String(ch.status||'pendente').toLowerCase()})),...state.parceladaItens.filter(p=>matches(p.data_vencimento)).map(p=>({valor:Number(p.valor)||0,status:String(p.status||'pendente').toLowerCase()}))];const all=rows.reduce((s,c)=>s+Number(c.valor||0),0),pg=rows.filter(isPaid).reduce((s,c)=>s+Number(c.valor||0),0);arr.push({l:d.toLocaleDateString('pt-BR',{month:'short'}).replace('.',''),all,pg})}const max=Math.max(1,...arr.flatMap(x=>[x.all,x.pg]));return `<div class="chart">${arr.map(x=>`<div class="bar-col"><div class="bars"><div class="bar income" title="Pagas ${money(x.pg)}" style="height:${Math.max(2,x.pg/max*100)}%"></div><div class="bar expense" title="Total ${money(x.all)}" style="height:${Math.max(2,x.all/max*100)}%"></div></div><div class="bar-label">${x.l}</div></div>`).join('')}</div>`}function contaStatus(c){
...
function contas(){
 ...
 const virtualSubsAll=state.assinaturaCobrancas.map(ch=>{const sub=state.assinaturas.find(s=>s.id===ch.assinatura_id);return {id:`subcharge-${ch.id}`,descricao:sub?.servico||'Assinatura',valor:Number(ch.valor)||0,vencimento:ch.data_vencimento,status:String(ch.status||'pendente').toLowerCase(),origem:'Assinatura',categoria_id:sub?.categoria_id||null,virtual:true}});
 const virtualLoansAll=state.parcelas.map(p=>{const loan=state.emprestimos.find(l=>l.id===p.emprestimo_id);return {id:`loan-${p.id}`,descricao:loan?loan.descricao:'Empréstimo',valor:Number(p.valor)||0,vencimento:p.data_vencimento,status:String(p.status||'pendente').toLowerCase(),origem:'Empréstimo',categoria_id:loan?.categoria_id||null,virtual:true,loanId:p.emprestimo_id,numero_parcela:p.numero_parcela}});
 const virtualParceladasAll=state.parceladaItens.map(p=>{const parent=state.parceladas.find(x=>x.id===p.conta_parcelada_id);return {id:`parcelada-${p.id}`,descricao:parent?.descricao||'Conta parcelada',valor:Number(p.valor)||0,vencimento:p.data_vencimento,status:String(p.status||'pendente').toLowerCase(),origem:'Parcelada',categoria_id:parent?.categoria_id||null,virtual:true,parceladaItemId:p.id,numero_parcela:p.numero_parcela}});
 ...
 const isOverdueRow=x=>!isPaid(x)&&new Date(String(x.vencimento||'')+'T12:00:00')<todayDate;
 const isNotDueRow=x=>!isPaid(x)&&!isOverdueRow(x);
 if(contaFilter==='paid')rows=rows.filter(isPaid);
 if(contaFilter==='pending')rows=rows.filter(isNotDueRow);
 if(contaFilter==='overdue')rows=rows.filter(isOverdueRow);
 if(contaFilter==='all' && contaListFilter.status!=='all'){
   rows=rows.filter(x=>contaListFilter.status==='paid'?isPaid(x):contaListFilter.status==='pending'?isNotDueRow(x):isOverdueRow(x));
   if(contaListFilter.origin!=='all')rows=rows.filter(x=>x.origem===contaListFilter.origin);
   if(contaListFilter.category!=='all')rows=rows.filter(x=>String(x.categoria_id||'')===String(contaListFilter.category));
 }else if(contaFilter==='all'){
   if(contaListFilter.origin!=='all')rows=rows.filter(x=>x.origem===contaListFilter.origin);
   if(contaListFilter.category!=='all')rows=rows.filter(x=>String(x.categoria_id||'')===String(contaListFilter.category));
 }
 const sortMode=contaListFilter.sort;
 rows.sort((a,b)=>{
   if(contaFilter==='all' && sortMode==='date-desc'){
     const am=String(a.vencimento||'').slice(0,7),bm=String(b.vencimento||'').slice(0,7),nowKey=monthKey(new Date()).slice(0,7);
     const rank=x=>x===nowKey?0:(x>nowKey?1:2),ra=rank(am),rb=rank(bm);
     if(ra!==rb)return ra-rb;
     return (ra===1||ra===2?String(b.vencimento).localeCompare(String(a.vencimento)):String(a.vencimento).localeCompare(String(b.vencimento)))||String(a.descricao).localeCompare(String(b.descricao),'pt-BR');
   }
   if(sortMode==='date-asc')return String(a.vencimento||'').localeCompare(String(b.vencimento||''))||String(a.descricao).localeCompare(String(b.descricao),'pt-BR');
   if(sortMode==='name-asc')return String(a.descricao||'').localeCompare(String(b.descricao||''),'pt-BR')||String(a.vencimento||'').localeCompare(String(b.vencimento||''));
   if(sortMode==='value-desc')return Number(b.valor||0)-Number(a.valor||0)||String(a.descricao||'').localeCompare(String(b.descricao||''),'pt-BR');
   if(sortMode==='origin-asc')return String(a.origem||'').localeCompare(String(b.origem||''),'pt-BR')||String(a.descricao||'').localeCompare(String(b.descricao||''),'pt-BR');
   return String(b.vencimento||'').localeCompare(String(a.vencimento||''))||String(a.descricao||'').localeCompare(String(b.descricao||''),'pt-BR');
 });
 const selectedRows=[...state.contas.filter(c=>inSelectedMonth(c.vencimento)),...virtualSubs,...virtualLoans,...virtualParceladas],paid=selectedRows.filter(isPaid).length,notDue=selectedRows.filter(isNotDueRow).length,overdue=selectedRows.filter(isOverdueRow).length;
 const origins=['Conta','Recorrente','Assinatura','Empréstimo','Parcelada'];
 const activeFilters=[contaListFilter.status!=='all',contaListFilter.origin!=='all',contaListFilter.sort!=='date-desc'].filter(Boolean).length;
 const filterMenu=contaFilter==='all'?`<div class="conta-filter-wrap"><button class="filter-trigger ${activeFilters?'has-filter':''}" data-action="toggle-conta-filter" aria-expanded="${contaFilterPanelOpen}">${icon('sliders-horizontal',15)} Filtrar${activeFilters?` <span class="filter-count">${activeFilters}</span>`:''}</button>${contaFilterPanelOpen?`<div class="conta-filter-menu" role="menu"><div class="filter-group"><strong>Status</strong><button data-conta-menu-status="all" class="${contaListFilter.status==='all'?'selected':''}">${contaListFilter.status==='all'?icon('check',14):''}<span>Todos</span></button><button data-conta-menu-status="paid" class="${contaListFilter.status==='paid'?'selected':''}">${contaListFilter.status==='paid'?icon('check',14):''}<span>Pagos</span></button><button data-conta-menu-status="pending" class="${contaListFilter.status==='pending'?'selected':''}">${contaListFilter.status==='pending'?icon('check',14):''}<span>Não vencidos</span></button><button data-conta-menu-status="overdue" class="${contaListFilter.status==='overdue'?'selected':''}">${contaListFilter.status==='overdue'?icon('check',14):''}<span>Vencidos</span></button></div><div class="filter-group"><strong>Origem</strong><button data-conta-menu-origin="all" class="${contaListFilter.origin==='all'?'selected':''}">${contaListFilter.origin==='all'?icon('check',14):''}<span>Todas</span></button>${origins.map(o=>`<button data-conta-menu-origin="${esc(o)}" class="${contaListFilter.origin===o?'selected':''}">${contaListFilter.origin===o?icon('check',14):''}<span>${esc(o)}</span></button>`).join('')}</div><div class="filter-group"><strong>Organizar</strong><button data-conta-menu-sort="date-desc" class="${contaListFilter.sort==='date-desc'?'selected':''}">${contaListFilter.sort==='date-desc'?icon('check',14):''}<span>Mais recentes</span></button><button data-conta-menu-sort="date-asc" class="${contaListFilter.sort==='date-asc'?'selected':''}">${contaListFilter.sort==='date-asc'?icon('check',14):''}<span>Mais antigas</span></button><button data-conta-menu-sort="name-asc" class="${contaListFilter.sort==='name-asc'?'selected':''}">${contaListFilter.sort==='name-asc'?icon('check',14):''}<span>Descrição (A–Z)</span></button><button data-conta-menu-sort="value-desc" class="${contaListFilter.sort==='value-desc'?'selected':''}">${contaListFilter.sort==='value-desc'?icon('check',14):''}<span>Maior valor</span></button><button data-conta-menu-sort="origin-asc" class="${contaListFilter.sort==='origin-asc'?'selected':''}">${contaListFilter.sort==='origin-asc'?icon('check',14):''}<span>Origem (A–Z)</span></button></div><div class="filter-footer"><button class="link-btn" data-action="clear-conta-filter">Limpar filtros</button></div></div>`:''}</div>`:'';
 return `<div class="toolbar"><div><div class="eyebrow">Planejamento mensal</div><h1>Contas</h1><p class="muted">${contaFilter==='all'?'Histórico completo de contas, com o mês atual sempre no topo.':`Lançamentos de ${monthName(selected)}`}</p></div><button class="btn btn-primary" data-action="new-conta">${icon('plus',17)} Lançar conta</button></div><div class="toolbar"><div class="toolbar-left"><button class="tab ${contaFilter==='all'?'active':''}" data-conta-filter="all">Todas (${state.contas.length+state.assinaturaCobrancas.length+state.parcelas.length+state.parceladaItens.length})</button><button class="tab ${contaFilter==='paid'?'active':''}" data-conta-filter="paid">Pagas (${paid})</button><button class="tab ${contaFilter==='pending'?'active':''}" data-conta-filter="pending">Pendentes (${notDue})</button><button class="tab ${contaFilter==='overdue'?'active':''}" data-conta-filter="overdue">Vencidas (${overdue})</button>${filterMenu}</div><input class="search" id="contaSearch" placeholder="Buscar conta, assinatura ou parcelada..."></div><section class="card section-card table-wrap"><table class="data-table"><thead><tr><th>Descrição</th><th>Origem</th><th>Valor</th><th>Vencimento</th><th>Status</th></tr></thead><tbody>${rows.length?rows.map(c=>{const st=contaStatus(c);return `<tr class="${!c.virtual?'clickable-row':''}" ${!c.virtual?`data-edit-conta="${c.id}" tabindex="0" role="button"`:''}><td data-label="Descrição"><strong>${esc(c.descricao)}</strong>${c.numero_parcela?`<br><small class="muted">Parcela ${c.numero_parcela}</small>`:''}${c.recorrente?`<br><small class="muted">${icon('repeat-2',11)} Conta recorrente</small>`:''}</td><td data-label="Origem"><span class="status ${c.origem==='Assinatura'?'gold':c.origem==='Empréstimo'?'blue':c.origem==='Parcelada'?'purple':'soft'}">${esc(c.origem||'Conta')}</span></td><td data-label="Valor" class="money">${money(c.valor)}</td><td data-label="Vencimento">${dateBR(c.vencimento)}</td><td data-label="Status"><button class="status ${st.cls}" data-toggle-status="${c.virtual?'':c.id}" ${c.virtual?'disabled':''}>${st.label}</button></td></tr>`}).join(''):'<tr><td colspan="5" class="empty">Nenhuma conta encontrada.</td></tr>'}</tbody></table></section>`;
}
...
function modal(title,body,extraClass=''){$('#modalRoot').innerHTML=`<div class="overlay" id="overlay"><div class="modal ${extraClass}"><div class="modal-head"><h2>${title}</h2><button class="close" id="closeModal">${icon('x',18)}</button></div>${body}</div></div>`;refreshIcons();$('#closeModal').onclick=closeModal;$('#overlay').addEventListener('click',e=>{if(e.target.id==='overlay')closeModal()})}
...
function openParceladaForm(p=null){
 modal(p?'Editar conta parcelada':'Nova conta parcelada',parceladaForm(p));
 bindMoneyInputs($('#parceladaForm'));
 $('#cancelForm').onclick=closeModal;
 const total=$('#parceladaForm [name="valor_total"]'),qty=$('#parceladaForm [name="quantidade_parcelas"]'),inst=$('#parceladaForm [name="valor_parcela"]');
 const sync=()=>{const q=Number(qty.value)||0,t=parseMoney(total.value);inst.value=q&&t?money(Math.round((t/q)*100)/100):'R$ 0,00';};
 [total,qty].forEach(x=>x?.addEventListener('input',sync));
 sync();
 $('#deleteParcelada')?.addEventListener('click',async()=>{if(!confirm('Excluir esta conta parcelada e todas as suas parcelas?'))return;const d=await sb.from('contas_parceladas').delete().eq('id',p.id).eq('familia_id',activeFamily?.id);if(d.error)return toast(d.error.message);closeModal();await loadData();render();toast('Conta parcelada excluída.');});
 $('#parceladaForm').onsubmit=async e=>{
  e.preventDefault();
  const f=new FormData(e.currentTarget),descricao=String(f.get('descricao')||'').trim(),q=Number(f.get('quantidade_parcelas')),t=parseMoney(f.get('valor_total')),vp=q&&t?Math.round((t/q)*100)/100:0;
  if(!descricao||q<1||t<=0||vp<=0)return toast('Informe descrição, valor total e quantidade de parcelas.');
  const payload={usuario_id:user.id,familia_id:activeFamily?.id,categoria_id:f.get('categoria_id')||null,descricao,valor_total:t,quantidade_parcelas:q,valor_parcela:vp,primeira_vencimento:f.get('primeira_vencimento'),status:String(f.get('status')||'Ativo'),observacao:f.get('observacao')||null,atualizado_em:new Date().toISOString()};
  let qdb=p?await sb.from('contas_parceladas').update(payload).eq('id',p.id).eq('familia_id',activeFamily?.id).select('id').single():await sb.from('contas_parceladas').insert(payload).select('id').single();
  if(qdb.error)return toast(`Não foi possível salvar: ${qdb.error.message}`);
  const id=qdb.data.id,first=new Date(payload.primeira_vencimento+'T12:00:00'),existing=p?state.parceladaItens.filter(x=>x.conta_parcelada_id===id):[];
  const items=Array.from({length:q},(_,i)=>{const d=new Date(first.getFullYear(),first.getMonth()+i,first.getDate()),prev=existing.find(x=>Number(x.numero_parcela)===i+1);return{conta_parcelada_id:id,usuario_id:user.id,familia_id:activeFamily?.id,numero_parcela:i+1,data_vencimento:iso(d),valor:vp,status:prev?.status||'pendente',valor_pago:Number(prev?.valor_pago)||0,data_pagamento:prev?.data_pagamento||null,observacao:prev?.observacao||null}});
  if(p){const del=await sb.from('contas_parceladas_itens').delete().eq('conta_parcelada_id',id).eq('familia_id',activeFamily?.id);if(del.error)return toast(`Conta salva, mas não foi possível recriar as parcelas: ${del.error.message}`);}
  const ins=await sb.from('contas_parceladas_itens').insert(items);
  if(ins.error)return toast(`Conta salva, mas as parcelas não foram criadas: ${ins.error.message}`);
  closeModal();await loadData();render();toast(p?'Conta parcelada atualizada.':'Conta parcelada criada.');
 };
}
...
function emprestimos(){
 ...
 return `<div class="toolbar"><div><div class="eyebrow">Crédito e compromissos</div><h1>Empréstimos</h1><p class="muted">Consulte os dados do empréstimo e abra as parcelas somente quando necessário.</p></div><button class="btn btn-primary" data-action="new-loan">${icon('plus',17)} Novo empréstimo</button></div><div class="toolbar-left"><button class="tab ${loanFilter==='Ativos'?'active':''}" data-loan-filter="Ativos">Ativos</button><button class="tab ${loanFilter==='Quitados'?'active':''}" data-loan-filter="Quitados">Quitados</button><button class="tab ${loanFilter==='Todos'?'active':''}" data-loan-filter="Todos">Todos</button></div><div class="loan-grid" style="margin-top:14px">${rows.length?rows.map(l=>{const ps=state.parcelas.filter(p=>p.emprestimo_id===l.id),pg=ps.filter(isPaid).length,pct=l.parcelas?Math.min(100,pg/l.parcelas*100):0,settled=loanIsSettled(l),displayStatus=loanDisplayStatus(l);return `<article class="card loan-card"><div class="loan-top"><div class="loan-icon">${icon('hand-coins',21)}</div><span class="status ${settled?'inactive':'active'}">${esc(displayStatus)}</span></div><h3 style="margin-top:13px">${esc(l.descricao)}</h3><p class="muted">${esc(l.credor)}</p><div class="money" style="font-size:22px;margin-top:10px">${money(l.total)}</div><p class="muted">${l.parcelas} parcelas • primeiro vencimento ${dateBR(l.primeira)}</p><div class="progress"><i style="width:${pct}%"></i></div><div class="loan-meta loan-card-actions"><span>${pg}/${l.parcelas||0} parcelas pagas</span><div class="loan-card-buttons"><button type="button" class="btn btn-soft btn-xs" data-open-loan="${l.id}">${icon('list-checks',13)} Ver Parcelas</button><button type="button" class="link-btn" data-edit-loan="${l.id}">Editar</button></div></div></article>`}).join(''):'<div class="card empty" style="grid-column:1/-1">Nenhum empréstimo cadastrado.</div>'}</div>`}
...
function bindLoanParcelActions(loanId){
 document.querySelectorAll('#overlay [data-loan-parcel-check]').forEach(ch=>ch.onchange=()=>{if(ch.checked)selectedLoanParcelIds.add(ch.dataset.loanParcelCheck);else selectedLoanParcelIds.delete(ch.dataset.loanParcelCheck);refreshLoanParcelSelectionUI(loanId)});
 ...
 $('#overlay [data-loan-select-all]')?.addEventListener('change',e=>{const ids=state.parcelas.filter(p=>p.emprestimo_id===loanId).map(p=>p.id);ids.forEach(id=>e.target.checked?selectedLoanParcelIds.add(id):selectedLoanParcelIds.delete(id));document.querySelectorAll('#overlay [data-loan-parcel-check]').forEach(ch=>{ch.checked=e.target.checked});refreshLoanParcelSelectionUI(loanId)});
}
function refreshLoanParcelSelectionUI(loanId){
 const ps=state.parcelas.filter(p=>p.emprestimo_id===loanId).sort((a,b)=>Number(a.numero_parcela)-Number(b.numero_parcela));
 const toolbar=$('#overlay .loan-parcel-toolbar');
 if(toolbar){toolbar.outerHTML=loanParcelToolbar(ps);
   $('#overlay [data-loan-bulk-pay]')?.addEventListener('click',()=>{const ids=[...selectedLoanParcelIds].filter(id=>state.parcelas.some(p=>p.id===id&&p.emprestimo_id===loanId));updateLoanParcels(ids,true,loanId)});
   $('#overlay [data-loan-bulk-pending]')?.addEventListener('click',()=>{const ids=[...selectedLoanParcelIds].filter(id=>state.parcelas.some(p=>p.id===id&&p.emprestimo_id===loanId));updateLoanParcels(ids,false,loanId)});
   $('#overlay [data-loan-bulk-delete]')?.addEventListener('click',()=>{const ids=[...selectedLoanParcelIds].filter(id=>state.parcelas.some(p=>p.id===id&&p.emprestimo_id===loanId));deleteLoanParcels(ids,loanId)});
 }
 const all=ps.length>0&&ps.every(p=>selectedLoanParcelIds.has(p.id));
 const selectAll=$('#overlay [data-loan-select-all]'); if(selectAll)selectAll.checked=all;
}
...
function editMeta(id){const m=state.metas.find(x=>x.id===id);if(!m)return;const items=state.metaItens.filter(i=>i.meta_id===id).sort((a,b)=>String(a.criado_em||'').localeCompare(String(b.criado_em||'')));modal(`Editar meta — ${esc(m.titulo)}`,`<form id="metaEditForm" class="form-grid"><div class="field full"><label>Nome da meta</label><input name="titulo" value="${esc(m.titulo||'')}" required maxlength="120"></div><div class="field full"><label>Etapas</label><div id="metaEditSteps">${items.map(i=>`<div class="meta-step-input" data-step-id="${i.id}"><input name="etapa" value="${esc(i.descricao||'')}" placeholder="Descreva a etapa" maxlength="180" required><button type="button" class="btn btn-outline" data-remove-edit-step aria-label="Excluir etapa">×</button></div>`).join('')||`<div class="meta-step-input"><input name="etapa" placeholder="Descreva a etapa" maxlength="180" required><button type="button" class="btn btn-outline" data-remove-edit-step aria-label="Excluir etapa">×</button></div>`}</div><button type="button" class="btn btn-outline" id="addEditMetaStep">+ Adicionar etapa</button></div><div class="form-actions"><button type="button" class="btn btn-outline" id="cancelForm">Cancelar</button><button class="btn btn-primary">Salvar alterações</button></div></form>`);const steps=$('#metaEditSteps');$('#cancelForm').onclick=closeModal;$('#addEditMetaStep').onclick=()=>steps.insertAdjacentHTML('beforeend',`<div class="meta-step-input"><input name="etapa" placeholder="Descreva a etapa" maxlength="180" required><button type="button" class="btn btn-outline" data-remove-edit-step aria-label="Excluir etapa">×</button></div>`);steps.onclick=e=>{const b=e.target.closest('[data-remove-edit-step]');if(!b)return;if(steps.children.length<=1)return toast('A meta precisa ter pelo menos uma etapa.');b.parentElement.remove()};$('#metaEditForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),titulo=String(f.get('titulo')||'').trim(),nodes=[...steps.querySelectorAll('.meta-step-input')],etapas=nodes.map(el=>({id:el.dataset.stepId||null,descricao:String(el.querySelector('input')?.value||'').trim()})).filter(x=>x.descricao);if(!titulo||!etapas.length)return toast('Informe o nome da meta e pelo menos uma etapa.');const u=await sb.from('metas').update({titulo,atualizado_em:new Date().toISOString()}).eq('id',id).eq('usuario_id',user.id);if(u.error)return toast('Não foi possível atualizar a meta: '+u.error.message);const keepIds=etapas.map(x=>x.id).filter(Boolean);const removed=items.filter(i=>!keepIds.includes(i.id)).map(i=>i.id);if(removed.length){const d=await sb.from('meta_itens').delete().in('id',removed).eq('meta_id',id).eq('usuario_id',user.id);if(d.error)return toast('Meta atualizada, mas não foi possível excluir algumas etapas: '+d.error.message)}for(const item of etapas.filter(x=>x.id)){const r=await sb.from('meta_itens').update({descricao:item.descricao,atualizado_em:new Date().toISOString()}).eq('id',item.id).eq('meta_id',id).eq('usuario_id',user.id);if(r.error)return toast('Meta atualizada, mas não foi possível atualizar uma etapa: '+r.error.message)}const novos=etapas.filter(x=>!x.id).map(x=>({meta_id:id,usuario_id:user.id,familia_id:activeFamily.id,descricao:x.descricao,concluida:false}));if(novos.length){const ins=await sb.from('meta_itens').insert(novos);if(ins.error)return toast('Meta atualizada, mas não foi possível adicionar novas etapas: '+ins.error.message)}closeModal();await loadData();render();toast('Meta atualizada com sucesso.')}}
