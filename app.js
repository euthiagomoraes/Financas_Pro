...
const state={contas:[],recorrentes:[],emprestimos:[],parcelas:[],categorias:[],assinaturas:[],assinaturaCobrancas:[]};
const SERVICE_LOGOS={
...
function contas(){
 ...
 const virtualSubs=state.assinaturaCobrancas.filter(ch=>inMonth(ch.data_vencimento)).map(ch=>{const sub=state.assinaturas.find(s=>s.id===ch.assinatura_id);return {id:`subcharge-${ch.id}`,descricao:sub?.servico||'Assinatura',valor:Number(ch.valor),vencimento:ch.data_vencimento,status:String(ch.status||'pendente').toLowerCase(),origem:'Assinatura',origemId:ch.assinatura_id,chargeId:ch.id,virtual:true}});
...
<<<<<<< HEAD
function assinaturas(){const total=state.assinaturas.filter(s=>s.ativa).reduce((a,s)=>a+Number(s.valor||0),0);return `<div class="toolbar"><div><div class="eyebrow">Custos recorrentes</div><h1>Assinaturas</h1><p class="muted">Acompanhe as cobranças mensais e seus pagamentos.</p></div><button class="btn btn-primary" data-action="new-sub">${icon('plus',17)} Adicionar</button></div><div class="sub-total"><span class="muted">Total mensal ativo</span><strong>${money(total)}</strong><span class="muted">${state.assinaturas.filter(s=>s.ativa).length} assinatura(s) ativa(s)</span></div><div class="subscription-grid">${state.assinaturas.length?state.assinaturas.map(s=>{const charges=state.assinaturaCobrancas.filter(c=>c.assinatura_id===s.id).sort((a,b)=>String(b.competencia).localeCompare(String(a.competencia)));return `<article class="card sub-card"><div class="service-logo">${SERVICE_LOGOS[s.icone_slug]?`<img src="${SERVICE_LOGOS[s.icone_slug]}" alt="${esc(s.servico)}" onerror="this.style.display='none'">`:icon('repeat-2',24)}</div><div class="sub-info"><h3>${esc(s.servico)}</h3><div class="muted">${money(s.valor)}/mês • vencimento dia ${s.dia_vencimento}</div><h4 style="margin:14px 0 8px">Histórico de cobranças</h4>${charges.length?`<div class="table-wrap"><table class="data-table"><thead><tr><th>Competência</th><th>Vencimento</th><th>Valor</th><th>Status</th><th>Pagamento</th><th></th></tr></thead><tbody>${charges.map(c=>`<tr><td>${String(c.competencia).slice(5,7)}/${String(c.competencia).slice(0,4)}</td><td>${dateBR(c.data_vencimento)}</td><td>${money(c.valor)}</td><td><span class="status ${c.status==='pago'?'active':c.status==='cancelado'?'inactive':'pending'}">${c.status==='pago'?'Pago':c.status==='cancelado'?'Cancelado':'Pendente'}</span></td><td>${c.data_pagamento?dateBR(String(c.data_pagamento).slice(0,10)):'—'}</td><td>${c.status==='pendente'?`<button class="btn btn-primary" data-pay-sub-charge="${c.id}">Marcar como pago</button>`:''}</td></tr>`).join('')}</tbody></table></div>`:'<p class="muted">Nenhuma cobrança registrada ainda.</p>'}</div><div><span class="status ${s.ativa?'active':'inactive'}">${s.ativa?'Ativa':'Inativa'}</span><div class="sub-actions" style="margin-top:8px"><button class="btn btn-danger" data-delete-sub="${s.id}" title="Excluir assinatura">${icon('trash-2',14)}</button></div></div></article>`}).join(''):'<div class="card empty" style="grid-column:1/-1">Nenhuma assinatura cadastrada. Adicione seus serviços recorrentes para acompanhar o custo mensal.</div>'}</div>`}
...
async function loadData(){
 ...
 const [c,l,p,sub,charges]=await Promise.all([
 ...
 const errs=[c,l,p,sub,charges].filter(x=>x.error);if(errs.length)console.warn('Supabase:',errs.map(x=>x.error.message));
}
