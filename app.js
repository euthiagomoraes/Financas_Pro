...
function bindPage(){
  ...
  $$("[data-delete-conta]").forEach(b=>b.onclick=()=>{if(confirm("Excluir esta conta?")){state.contas=state.contas.filter(x=>x.id!=b.dataset.deleteConta);save();render();toast("Conta excluÃ­da")}});
  $$("[data-pay]").forEach(b=>b.onclick=()=>payConta(b.dataset.pay));
  $$("[data-delete-loan]").forEach(b=>b.onclick=()=>{if(confirm("Excluir este emprÃ©stimo e suas parcelas?")){state.emprestimos=state.emprestimos.filter(x=>x.id!=b.dataset.deleteLoan);save();render();toast("EmprÃ©stimo excluÃ­do")}});
