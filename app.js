...
async function enablePushNotifications(){
 if(!('serviceWorker' in navigator)||!('PushManager' in window)||!(window.FINANCAS_VAPID_PUBLIC_KEY||window.FINANCAS_CONFIG?.vapidPublicKey||'').trim()==='') return toast('Configure a chave pública VAPID no index.html.');
 try{const reg=await navigator.serviceWorker.register('./sw.js');const permission=await Notification.requestPermission();if(permission!=='granted')return toast('Permissão de notificações não concedida.');let sub=await reg.pushManager.getSubscription();if(!sub)sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:urlBase64ToUint8Array(vapidKey)});const json=sub.toJSON();const {error}=await sb.from('push_subscriptions').upsert({user_id:user.id,endpoint:json.endpoint,p256dh:json.keys.p256dh,auth:json.keys.auth,user_agent:navigator.userAgent,updated_at:new Date().toISOString()},{onConflict:'endpoint'});if(error)return toast('Erro ao salvar dispositivo: '+error.message);toast('Notificações ativadas neste celular.');}catch(e){console.error(e);toast('Não foi possível ativar as notificações.');}}
...
function toggleSidebarCollapse(){document.body.classList.toggle('sidebar-collapsed');const b=$('#sidebarCollapse');if(b){const collapsed=document.body.classList.contains('sidebar-collapsed');b.innerHTML=icon(collapsed?'panel-left-open':'panel-left-close',18);b.title=collapsed?'Expandir menu':'Recolher menu';refreshIcons();localStorage.setItem('financas-sidebar-collapsed',collapsed?'1':'0');}}
