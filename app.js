...
function contas(){
 ...
 if(contaFilter==='paid')rows=rows.filter(isPaid); if(contaFilter==='pending')rows=rows.filter(x=>!isPaid(x)&&new Date(String(x.vencimento)+'T12:00:00')>=new Date(new Date().setHours(0,0,0,0))); if(contaFilter==='overdue')rows=rows.filter(x=>!isPaid(x)&&new Date(String(x.vencimento)+'T12:00:00')<new Date(new Date().setHours(0,0,0,0)));
