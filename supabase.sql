...
-- ============================================================
-- FINANÇAS PRO — CONTAS: CRUD COMPARTILHADO POR FAMÍLIA
-- ============================================================
...
as $$
  select exists (
    ...
    where m.familia_id = target_family and m.usuario_id = auth.uid()
  );
$$;
...
begin
  for r in select distinct usuario_id from public.contas where familia_id is null and usuario_id is not null loop
    select m.familia_id into f
    from public.familia_membros m
    where m.usuario_id = r.usuario_id
    order by m.criado_em limit 1;
    if f is not null then
      update public.contas set familia_id=f
      where usuario_id=r.usuario_id and familia_id is null;
    end if;
  end loop;
end $$;
...
create policy contas_select_family on public.contas
for select to authenticated
using (public.is_family_member(familia_id));

create policy contas_insert_family on public.contas
for insert to authenticated
with check (public.is_family_member(familia_id) and usuario_id = auth.uid());

create policy contas_update_family on public.contas
for update to authenticated
using (public.is_family_member(familia_id))
with check (public.is_family_member(familia_id));

create policy contas_delete_family on public.contas
